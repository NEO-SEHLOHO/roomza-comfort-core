...
const modalStates = {
  ...
  status: {
    ...
    body: "Track the residences you have applied for and see the latest status saved by the accommodation team.",
    submit: "",
    note: "Statuses update when the residence team reviews your application.",
  },
  login: {
...
function countLocalApplications(user) {
  ...
  return applicationsForUser(applications, user).length;
}
...
function formatApplicationDate(value) {
  ...
  return rawDate.toLocaleDateString("en-ZA", {
    day: "2-digit",
    month: "short",
    year: "numeric",
  });
...
function normaliseApplication(application) {
  ...
  const status = application.applicationStatus || application.status || "Submitted";

  return {
    id: application.id || "",
    title,
    campus: application.campus || listing.campus || "",
    rent: application.rent || listing.price || 0,
    status,
    submittedAt: formatApplicationDate(application.timestamp || application.createdAt || application.applicationDate),
...
function statusClass(status) {
  ...
  if (statusText.includes("approved") || statusText.includes("accepted")) return "is-approved";
  if (statusText.includes("declined") || statusText.includes("rejected")) return "is-declined";
  if (statusText.includes("review") || statusText.includes("pending")) return "is-review";
  return "is-submitted";
}
  ...
  if (!applications.length) {
    statusList.innerHTML = `
      <div class="status-empty">
        <h3>No applications found yet.</h3>
        <p>When you apply for a residence, it will appear here with its latest status.</p>
      </div>
    `;
    return;
  }

  statusList.innerHTML = applications
    .map((application) => {
      const item = normaliseApplication(application);
      const details = [
        item.campus,
        item.rent ? `${formatMoney(item.rent)} pm` : "",
      ...
      return `
        <article class="status-card">
          <div>
            <p class="status-card-eyebrow">${escapeHtml(details.join(" - ") || "Residence application")}</p>
            <h3>${escapeHtml(item.title)}</h3>
            ${item.studentNumber ? `<p>Student number: ${escapeHtml(item.studentNumber)}</p>` : ""}
          </div>
...
async function addApplicationQueryDocs(query, docs, label) {
  try {
    const snapshot = await query.get();
    snapshot.docs.forEach((doc) => {
      docs.set(doc.id, { id: doc.id, ...doc.data() });
    });
    console.debug(`Application status ${label} query returned ${snapshot.docs.length} documents.`);
    return true;
  } catch (error) {
    console.warn(`Could not load applications by ${label}.`, error);
    return false;
  }
}
...
function applicationDateMs(application) {
  const value = application.timestamp || application.createdAt || application.applicationDate;
  if (!value) return 0;
...
function sortApplications(applications) {
  return [...applications].sort((first, second) => {
    return applicationDateMs(second) - applicationDateMs(first);
  });
}

async function loadApplicationsForUser(user) {
  if (!user) return [];

  const services = firebaseServices();
  if (!services?.db) {
    const store = readDemoStore();
    return sortApplications(applicationsForUser(store.collections?.residence_applications || [], user));
  }
  ...
  if (user.uid) {
    const ok = await addApplicationQueryDocs(baseRef.where("userId", "==", user.uid), docs, "userId");
    successfulQueries += ok ? 1 : 0;
  }

  if (user.email) {
    const ok = await addApplicationQueryDocs(baseRef.where("email", "==", user.email), docs, "email");
    successfulQueries += ok ? 1 : 0;
  }
  ...
  return sortApplications(Array.from(docs.values()));
}
...
function configureModalForm(action) {
  ...
  if (modalForm) modalForm.hidden = isStatus;
  if (statusPanel) statusPanel.hidden = !isStatus;

  setFieldVisible(modalNameField, !isStatus && (!isAuth || isRegister) && !isSignedInDetails, !isDetails);
  setFieldVisible(modalEmailField, !isStatus && !isSignedInDetails, true);
  setFieldVisible(modalPasswordField, !isStatus && isAuth, true);
  setFieldVisible(modalPhoneField, !isStatus && (!isAuth || isRegister) && !isSignedInDetails, isRegister || isApplication);
...
async function openStatusModal() {
  ...
  if (statusList) {
    statusList.innerHTML = '<p class="status-message">Checking applications...</p>';
  }

  try {
    const applications = await loadApplicationsForUser(signedInUser);
    ...
    if (statusList) {
      statusList.innerHTML = `
        <div class="status-empty">
          ...
          <p>Check your connection or Firestore rules, then try again.</p>
        </div>
      `;
    }
  }
