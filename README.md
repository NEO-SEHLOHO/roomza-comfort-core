...
Login uses email and password only. Registration collects name, email, phone number, and password, then creates a Firebase Auth user and stores the student profile in Firestore under `users`. The header listens to Firebase Auth state and changes from `Login` to signed-in account actions with `Logout`.
...
Signed-in students can click the application-count button in the header to view every residence they applied for, including each saved `applicationStatus`.
